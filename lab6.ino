...
Stepper myStepper(stepsPerRevolution, 11, 9, 10, 8);// define which pins are used
Servo myservo; 
...
// Game variables
int score1 = 0; // firstly both scores are 0
int score2 = 0;
...
void setup() {
  ...
  myStepper.setSpeed(1000); //if we increase it motor spins faster ,lose steps,not rotate properly , if we decrease it motor spins slower,more stable and accurate steps

  myservo.attach(servoPin); // this line is connected with 7

  pinMode(buttonPlayer1, INPUT_PULLUP); //by default set to HIGH
  pinMode(buttonPlayer2, INPUT_PULLUP);
  pinMode(buzzer, OUTPUT);

  randomSeed(analogRead(A0)); // random generate between 1 to 20
...
void loop() {
  ...
  delay(5000); //If you increase it: Longer pause before next game. If you decrease, it: Game restarts faster.
}
